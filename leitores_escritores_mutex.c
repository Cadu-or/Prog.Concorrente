/*
	...
	Estudo dirigido 2 - Problema dos leitores e escritores
*/
...
void * reader(void *arg){
    ...
    while(TRUE){                      			/* repete para sempre */
		...
		pthread_mutex_lock(&lock_ne);		/* Se existe escritor na posse do turno ele nao roda */
...
void * writer(void *arg) {
	...
	while(TRUE){                          	/* Repete para sempre */
		...
		pthread_mutex_lock(&lock_ne);		/* Trava pois existe leitores querendo escrever */
            pthread_mutex_lock(&lock_bd);
            	write_data_base(i);       	/* Atualiza os dados */
				num_escritores--;			/* Decrementa por ja ter usado um escritor */
	    	pthread_mutex_unlock(&lock_bd);

		pthread_mutex_unlock(&lock_ne);		/* Libera para os leitores conseguirem fazer suas funcoes */
    }
